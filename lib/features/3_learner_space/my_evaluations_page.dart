// lib/features/3_learner_space/my_evaluations_page.dart
        ...
        body: BlocBuilder<SubmissionsBloc, SubmissionsState>(
          builder: (context, state) {
            ...
            if (state is SubmissionsLoaded) {
              // On filtre pour ne garder que les évaluations notées
              final gradedEvaluations = state.submissions
                  .where(
                    (s) =>
                        s.status == 'graded' &&
              ...
              if (gradedEvaluations.isEmpty) {
                return const Center(
                  child: Text(
                    "Vous n'avez aucune évaluation notée pour le moment.",
                  ),
                );
              }
              return RefreshIndicator(
                ...
                child: ListView.builder(
                  itemCount: gradedEvaluations.length,
                  itemBuilder: (context, index) {
                    final submission = gradedEvaluations[index];
                    return Card(

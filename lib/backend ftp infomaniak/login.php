...
if (!empty($data->email) && !empty($data->password)) {
    ...
    // On sélectionne maintenant aussi la colonne 'profile_image_url'.
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($data->password, $hashed_password)) {
            ...
            // On ajoute le rôle et l'URL de l'image de profil aux données renvoyées.
            $user_data = [
                "id" => $user['id'],
                "name" => $user['name'],
                "email" => $user['email'],
                "role" => $user['role'],
                "profile_image_url" => $user['profile_image_url'] // Ajout de l'URL
            ];
